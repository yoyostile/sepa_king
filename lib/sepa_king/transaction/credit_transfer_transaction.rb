...
module SEPA
  class CreditTransferTransaction < Transaction
    attr_accessor :instructed_priority,
                  :service_level,
                  :creditor_address,
                  :category_purpose

    validates_inclusion_of :instructed_priority, :in => %w(NORM HIGH), :allow_nil => true
    validates_inclusion_of :service_level, :in => %w(SEPA NURG URGP), :allow_nil => true
    ...
    def schema_compatible?(schema_name)
      ...
      when PAIN_001_001_03
        !self.service_level || (self.service_level == 'SEPA' && self.currency == 'EUR') ||
          (self.service_level == 'NURG' && self.currency == 'GBP')
